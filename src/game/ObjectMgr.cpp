...
Guild * ObjectMgr::GetGuildById(uint32 GuildId)
{
	if (GuildId==0)
		return NULL;
	...
	if (!sWorld.getConfig(CONFIG_PRECACHE_GUILDS))
...
Guild * ObjectMgr::GetGuildByName(const std::string& guildname)
{
	...
	if (!sWorld.getConfig(CONFIG_PRECACHE_GUILDS))
		return LoadGuildByName(guildname);
	else
		return NULL;
}

std::string ObjectMgr::GetGuildNameById(uint32 GuildId)
{
	if (GuildId==0)
		return "";
	...
	if (!sWorld.getConfig(CONFIG_PRECACHE_GUILDS))
	{
		Guild* guild = LoadGuildById(GuildId);
		if(guild)
...
Guild* ObjectMgr::GetGuildByLeader(const uint64 &guid)
{
    for (GuildMap::const_iterator itr = mGuildMap.begin(); itr != mGuildMap.end(); ++itr)
...
{
    //                                                             0             1           2                 3           4
    QueryResult_AutoPtr result  = CharacterDatabase.Query("SELECT checktype, check_period,alarmscount, disableoperation, messagenum,"
    //                                                             5          6             7             8
...
Guild* ObjectMgr::LoadGuildById(uint32 guildid)
{
    if (guildid==0)
        return NULL;
    ...
    //                                                    0             1          2          3           4           5           6
    QueryResult_AutoPtr result = CharacterDatabase.PQuery("SELECT guild.guildid,guild.name,leaderguid,EmblemStyle,EmblemColor,BorderStyle,BorderColor,"
    //   7               8    9    10         11        12
        "BackgroundColor,info,motd,createdate,BankMoney,COUNT(guild_bank_tab.guildid) "
        "FROM guild LEFT JOIN guild_bank_tab ON guild.guildid = guild_bank_tab.guildid WHERE guild.guildid='%u'", guildid);
    ...
    // load guild ranks
    //                                                                0       1   2     3      4
    QueryResult_AutoPtr guildRanksResult   = CharacterDatabase.PQuery("SELECT guildid,rid,rname,rights,BankMoneyPerDay FROM guild_rank WHERE guildid='%u'", guildid);

    // load guild members
    //                                                                0       1                 2    3     4       5                  6
    QueryResult_AutoPtr guildMembersResult = CharacterDatabase.PQuery("SELECT guildid,guild_member.guid,rank,pnote,offnote,BankResetTimeMoney,BankRemMoney,"
    //   7                 8                9                 10               11                12
    ...
    //                                                                      0       1     2   3       4
    QueryResult_AutoPtr guildBankTabRightsResult = CharacterDatabase.PQuery("SELECT guildid,TabId,rid,gbright,SlotPerDay FROM guild_bank_right WHERE guildid='%u'", guildid);
...
Guild* ObjectMgr::LoadGuildByName(const std::string& guildname)
{
    QueryResult_AutoPtr result = CharacterDatabase.PQuery( "SELECT guildid FROM guild WHERE name = '%s'" , guildname.c_str());
    if( !result )
        return NULL;
    Field *fields = result->Fetch();
...
Guild* ObjectMgr::LoadGuildByLeader(const uint64 &guid)
{
    QueryResult_AutoPtr result = CharacterDatabase.PQuery( "SELECT guildid FROM guild WHERE leaderguid = '%u'" , GUID_LOPART(guid));
    if( !result )
        return NULL;
    Field *fields = result->Fetch();
    uint32 guildid = fields[0].GetUInt32();
...
AreaTrigger const* ObjectMgr::GetGoBackTrigger(uint32 Map) const
{
    ...
    {
        if((!useParentDbValue && itr->second.target_mapId == mapEntry->entrance_map) || (useParentDbValue && itr->second.target_mapId == parentId))
        {
