...
static void AttemptJoin(Player* _player)
{
    ...
    for (HashMapHolder<Player>::MapType::const_iterator iter = players.begin(); iter != players.end(); ++iter)
    {
        ...
        {
            if (sWorld.getConfig(CONFIG_RESTRICTED_LFG_CHANNEL) && _player->GetSession()->GetSecurity() <= SEC_MODERATOR)
        ...
        {
            if (sWorld.getConfig(CONFIG_RESTRICTED_LFG_CHANNEL) && plr->GetSession()->GetSecurity() <= SEC_MODERATOR)
                plr->LeaveLFGChannel();
        }
    }
}
