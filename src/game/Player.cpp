...
bool Player::LoadFromDB(uint32 guid, SqlQueryHolder *holder)
{
    ...
    if (ObjectMgr::CheckPlayerName(m_name) != CHAR_NAME_SUCCESS ||
        (GetSession()->GetSecurity() <= SEC_MODERATOR && objmgr.IsReservedName(m_name)))
    {
...
void Player::_SaveAuras()
{
    ...
    for (AuraMap::const_iterator itr = m_ownedAuras.begin(); itr != m_ownedAuras.end() ; ++itr)
    {
        ...
        CharacterDatabase.PExecute("REPLACE INTO character_aura (guid,caster_guid,spell,effect_mask,recalculate_mask,stackcount,amount0,amount1,amount2,base_amount0,base_amount1,base_amount2,maxduration,remaintime,remaincharges) "
...
bool Player::BuyItemFromVendor(uint64 vendorguid, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    VendorItem * crItem = NULL;
    size_t vendor_slot = 0;

    int8 scriptreturn = sScriptMgr.ItemBuy(this, pCreature, item, count, &vendor_slot);

    if(scriptreturn == 0) { //Pruefung nicht bestanden
        return false;
    }
    else if(scriptreturn > 0) {
    //just dummy - makes it easier in code later
	crItem = new VendorItem(25, 0, 0, 0);
    }
    else { //keine Scriptpruefung
    ...
    vendor_slot = vItems->FindItemSlot(item);
    ...
    crItem = vItems->m_items[vendor_slot];
...
void Player::UpdateAreaDependentAuras(uint32 newArea)
{
    ...
    if (HasAura(49256))                 //Defending Wyrmrest Temple
        if (GetVehicle())
            if (newArea != 4254 && newArea != 4161 && newArea != 4181 && newArea != 4183 && newArea != 4176)
                ExitVehicle();
 }
