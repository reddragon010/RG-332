        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARLOCK:
            {
                ...
                else if (m_spellInfo->SpellFamilyFlags[1] & 0x400000)
                {
                    ...
                    {
                        // Return mana if improved felhunter
                        ...
                        if (owner && (owner->GetTypeId() == TYPEID_PLAYER))
                        {
                            if (AuraEffect * aurEff = owner->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER, SPELLFAMILY_WARLOCK, 214, 0))
                            {
                                // returns 4% or 8% mana
                                int32 amount = 5;
                                if (aurEff->GetSpellProto()->Id == 54038)
                                    amount = 9;
                                m_caster->CastCustomSpell(m_caster,54425,&amount,NULL,NULL,true,NULL,aurEff,m_caster->GetGUID());
                            }
                        }
            ...
            case SPELLFAMILY_HUNTER:
            {
                ...
                else if (m_spellInfo->SpellFamilyFlags[1] & 0x1)
                {
                    ...
                    if(m_caster->HasAura(56826)) //Glyph of Steady shot
                    {
                        Unit::AuraApplicationMap& Auras = unitTarget->GetAppliedAuras();
                        for (Unit::AuraApplicationMap::iterator i = Auras.begin(); i != Auras.end(); ++i)
                            if((*i).second->GetBase()->GetSpellProto()->SpellFamilyFlags[0] & 0x4000)
                                damage *= 1.1;
                    }
                }
                break;
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 52845:                                 // Brewfest Mount Transformation (Faction Swap)
                    ...
                    return;
                case 50546:                                 // Focus on the beach: Ley Line Focus Control Ring Effect
                    if (!m_originalCaster || !m_caster)
                        return;
                    m_caster->CastSpell(m_originalCaster, 47390, true);
                    return;
                case 50547:                                 // Atop the Woodlands: Ley Line Focus Control Amulet Effect
                    if (!m_originalCaster || !m_caster)
                        return;
                    m_caster->CastSpell(m_originalCaster, 47472, true);
                    return;
                case 50548:                                 // The End of the Line: Ley Line Focus Control Talisman Effect
                    if (!m_originalCaster || !m_caster)
                        return;
                    m_caster->CastSpell(m_originalCaster, 47635, true);
                    return;
                case 49349:                                 // Return to the Earth
                    if (!m_caster)
                        return;
                    m_caster->ToPlayer()->KilledMonsterCredit(27530, NULL);
                    return;
...
//              damage = m_caster->SpellDamageBonus(m_caster, m_spellInfo,uint32(damage > 0 ? damage : 0), SPELL_DIRECT_DAMAGE);
        ...
        case SPELLFAMILY_SHAMAN:
            ...
            if (m_spellInfo->SpellFamilyFlags[0] & SPELLFAMILYFLAG_SHAMAN_HEALING_STREAM)
            {
                ...
                if (Unit *owner = m_caster->GetOwner()) {
                    ...
                    if (AuraEffect* glyphdummy = owner->GetAuraEffect(55456, 0)) //Glyph of Healing Stream Totem
                        damage += damage * glyphdummy->GetAmount() / 100;
                }
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            if (m_spellInfo->Id == 49560)
            {
		        ...
		        if(unitTarget == m_caster) return; //possible fix
  
		        if (Unit *unit = unitTarget->GetVehicleBase())
                    return;
                if(unitTarget->GetTypeId() == TYPEID_UNIT && unitTarget->ToCreature()->isWorldBoss())
                {
                    return;
                }
...
void Spell::EffectCreateItem(uint32 i)
{
    switch(m_spellInfo->Id)
    {
        case 56230:                            // Create Fireforged Eyes
            if (m_originalCaster)
                unitTarget = m_originalCaster;
            break;
    }
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
                ...
                case 47097:                               //Surge Needle Teleporter
                    if (!m_caster || m_caster->GetMapId() != 571 || m_caster->GetAreaId() != 4157)
                        return;

                    if (m_caster->GetPositionZ() > 195 && m_caster->GetPositionZ() < 203  && m_caster->GetDistance(3338, 2517, 200) < 10)
                        m_caster->CastSpell(m_caster, 47324, false);
                    else 
                        m_caster->CastSpell(m_caster, 47325, false);
                    break;
            }
            break;
        }
