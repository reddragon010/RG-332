...
// < 0 -> Pruefung nicht vorhanden, normal weitermachen
// = 0 -> Pruefung nicht bestanden, nicht verkaufen
// > 0 -> Pruefung bestanden, verkaufen!
int8 ScriptMgr::ItemBuy(Player* pPlayer, Creature* pCreature, uint32 itemId, uint32 count, size_t* vendorslot)
{
    Script *tmpscript = m_scripts[pCreature->GetScriptId()];
    if (!tmpscript || !tmpscript->pItemBuy) return -1;

    pPlayer->PlayerTalkClass->ClearMenus();
    return tmpscript->pItemBuy(pPlayer, pCreature, itemId, count, vendorslot);
}
