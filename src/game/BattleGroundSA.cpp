...
bool BattleGroundSA::ResetObjs()
{
    ...
    for (uint8 i = 0; i < BG_SA_TELEPORTER_1; i++)
    {
    ...
    for (uint8 i = BG_SA_TELEPORTER_1; i < BG_SA_CENTRAL_FLAG; i++)
    {
    ...
    // Teleport Trigger
     ...
     for (uint8 i = BG_SA_NPC_TELEPORT_TRIGGER_1; i < BG_SA_MAXNPC; i++)
    {
...
void BattleGroundSA::CheckTanks()
{
    for (uint8 i = BG_SA_DEMOLISHER_1; i <= BG_SA_DEMOLISHER_4;i++)
    {
        if (Creature* dem = GetBGCreature(i))
			if(dem->GetHealth() == 0)
				if (DelCreature(i))
					if(AddCreature(BG_SA_NpcEntries[i], i, (attackers == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE), BG_SA_NpcSpawnlocs[i][0],BG_SA_NpcSpawnlocs[i][1],BG_SA_NpcSpawnlocs[i][2],BG_SA_NpcSpawnlocs[i][3]))
                        if (Creature* dem = GetBGCreature(i))
