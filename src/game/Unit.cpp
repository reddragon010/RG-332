        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
                ...
                aura->GetSpellProto()->SpellFamilyFlags[2] == 0 ) //it's better to check all the flags parts and not only [1]
            {
                if (AuraEffect const * aurEff = aura->GetEffect(0))
                {
                    ...
                    Unit *caster = aura->GetCaster();
                    dispeller->CastCustomSpell(dispeller, 31117, &damage, NULL, NULL, true, NULL, NULL, caster->GetGUID());
                }
            }
            // Flame Shock
        ...
        case SPELLFAMILY_DRUID:
        {
            switch(dummySpell->Id)
            {
                ...
                case 54832:
                {
                    ...
                    basepoints0 = uint32((GetCreateMana() * mana_perc / 100) / 10);
    ...
    for (AuraEffectList::const_iterator i = mModDamagePercentTaken.begin(); i != mModDamagePercentTaken.end(); ++i)
        if ((*i)->GetMiscValue() & GetSpellSchoolMask(spellProto))
        {
            if ((*i)->GetAmount() > 0)
            {
                switch ((*i)->GetId())
                {
                case 58105:                                         // SPELL_POWER_OF_SHADRON
                case 61248:                                         // SPELL_POWER_OF_TENEBRON
                case 61251:                                         // SPELL_POWER_OF_VESPERON
                case 57935:                                         // SPELL_TWILIGHT_TORMENT_VESP
                case 58835:                                         // SPELL_TWILIGHT_TORMENT_VESP_ACO
...
int32 Unit::SpellBaseDamageBonus(SpellSchoolMask schoolMask)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        DoneAdvertisedBenefit +=(this->getPowerType() == POWER_MANA) ? this->ToPlayer()->GetBaseSpellPowerBonus() : 0;
...
{
    // players, guardians?, pets, hunterpets, totems? can make critical hits with spells
    if (GetTypeId() != TYPEID_PLAYER && !isGuardian() && !isPet() && !isHunterPet() && !isTotem())
        return false;
...
int32 Unit::SpellBaseHealingBonus(SpellSchoolMask schoolMask)
{
    ...
    {
        // Base value (only for players that actually use mana as a source for abilities)
        AdvertisedBenefit +=(this->getPowerType() == POWER_MANA) ? this->ToPlayer()->GetBaseSpellPowerBonus() : 0;
