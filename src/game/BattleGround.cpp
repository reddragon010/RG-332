...
uint32 BattleGroundQuestIds[5][10]=
{
	{24224, 24225, 11342, 24219, 14180, 24218, 24216, 11338, 14183, 24217},
	{14181, 11339, 11335, 24226, 24223, 14178, 24220, 24221,     0,     0},
	{11340, 24427, 24426, 13428, 11336, 13427,     0,     0,     0,     0},
	{14182, 11341, 14179, 11337,     0,     0,     0,     0,     0,     0},
	{13407, 13405,     0,     0,     0,     0,     0,     0,     0,     0}
}; 
...
void BattleGround::RewardQuestComplete(Player *plr)
{
    ...
    switch(GetTypeID())
    {
        case BATTLEGROUND_AV:
			...
			for (uint8 i = 0; i < AV_QUEST_COUNT; ++i) {
				if(plr->IsActiveQuest(BattleGroundQuestIds[2][i]))
					plr->CompleteQuest(BattleGroundQuestIds[2][i]);
			}
            break;
        case BATTLEGROUND_WS:
            quest = SPELL_WS_QUEST_REWARD;
			for (uint8 i = 0; i < WS_QUEST_COUNT; ++i) {
				if(plr->IsActiveQuest(BattleGroundQuestIds[0][i]))
					plr->CompleteQuest(BattleGroundQuestIds[0][i]);
			}
            break;
        case BATTLEGROUND_AB:
            quest = SPELL_AB_QUEST_REWARD;
			for (uint8 i = 0; i < AB_QUEST_COUNT; ++i) {
				if(plr->IsActiveQuest(BattleGroundQuestIds[1][i]))
					plr->CompleteQuest(BattleGroundQuestIds[1][i]);
			}
            break;
        case BATTLEGROUND_EY:
            quest = SPELL_EY_QUEST_REWARD;
			for (uint8 i = 0; i < EY_QUEST_COUNT; ++i) {
				if(plr->IsActiveQuest(BattleGroundQuestIds[3][i]))
					plr->CompleteQuest(BattleGroundQuestIds[3][i]);
			}
            break;
        case BATTLEGROUND_SA:
            quest = SPELL_SA_QUEST_REWARD;
			for (uint8 i = 0; i < SA_QUEST_COUNT; ++i) {
				if(plr->IsActiveQuest(BattleGroundQuestIds[4][i]))
					plr->CompleteQuest(BattleGroundQuestIds[4][i]);
			}
            break;
        default:
            return;
