    ...
    *data << uint32(scoreCount);                            // placeholder
    ...
    for (BattleGround::BattleGroundScoreMap::const_iterator itr = itr2; itr != bg->GetPlayerScoresEnd();)
    {
        ...
        if (!bg->IsPlayerInBattleGround(itr2->first))
        {
            ...
            sLog.outError("Player %llu has scoreboard entry for battleground %u but is not in battleground!", itr->first, bg->GetTypeID());
            continue;
        }
        ...
        *data << (int32)itr2->second->DamageDone;             // damage done
        *data << (int32)itr2->second->HealingDone;            // healing done
