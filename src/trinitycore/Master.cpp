...
public:
    ...
    void checkping ()
    {
        ...
        if ((++loopCounter) == numLoops)
        {
           ...
           InternDatabase.Query ("SELECT 1 FROM jos_rgpremium_codes LIMIT 1");
        }
    }
...
bool Master::_StartDB()
{
    ...
    ///- Get intern database info from configuration file
    dbstring = sConfig.GetStringDefault("InternDatabaseInfo", "");
    if(dbstring.empty())
    {
        sLog.outError("Intern Database not specified in configuration file");
        return false;
    }

    ///- Initialise the intern database
    if(!InternDatabase.Initialize(dbstring.c_str()))
    {
        sLog.outError("Cannot connect to intern database %s",dbstring.c_str());
        return false;
    }
