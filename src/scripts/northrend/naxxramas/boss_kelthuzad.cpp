...
struct boss_kelthuzadAI : public BossAI
{
    ...
    uint64 uiShadowFissureTargetGuid;
    uint32 Phase;
    ...
    void Reset()
    {
        ...
        if (Unit* pTarget = Unit::GetUnit(*me, uiShadowFissureTargetGuid))
        {
            pTarget->RemoveAurasDueToSpell(46265);
            pTarget->RemoveAurasDueToSpell(69422);
        }
                ...
                switch(eventId)
                {
                    ...
                    case EVENT_FISSURE:
                        if (Unit *pTarget = SelectUnit(SELECT_TARGET_RANDOM,0))
                        {
							...
							pTarget->AddAura( 46265 , pTarget ); // workaround for visual
							pTarget->AddAura( 69422 , pTarget );
							uiShadowFissureTargetGuid = pTarget->GetGUID();
							events.ScheduleEvent(EVENT_REMOVE_FISSURE_AURA, 8*IN_MILISECONDS);
						}
                        events.RepeatEvent(urand(10000,45000));
                        break;
                    case EVENT_REMOVE_FISSURE_AURA:
						if (Unit* pTarget = Unit::GetUnit(*me, uiShadowFissureTargetGuid))
						{
							pTarget->RemoveAurasDueToSpell(46265);
							pTarget->RemoveAurasDueToSpell(69422);
...
enum bigglesworth
{
    TRIGGER_NPC                                              = 2230000, // Mr.Bigglesworth Trigger
};
...
{												
    ...
    void JustDied(Unit *killer)
    {
        ...
        Creature *Trigger = killer->SummonCreature(TRIGGER_NPC, pos, TEMPSUMMON_TIMED_DESPAWN, 5*IN_MILISECONDS);
        Trigger->SetName("Kel'Thuzad");
        DoScriptText(-1533089,Trigger);
    }
...
void AddSC_boss_kelthuzad()
{
	...
	newscript = new Script;
    newscript->Name = "npc_mr_bigglesworth";
    newscript->GetAI = &GetAI_npc_mr_bigglesworth;
    newscript->RegisterSelf();
}
