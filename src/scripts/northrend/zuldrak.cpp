...
/*####
## npc_heb_drakkar_troll
####*/

enum eHebdrakkar_troll
{
    QUEST_SCALPS = 12659,
...
struct npc_heb_drakkar_trollAI : public ScriptedAI
{
    npc_heb_drakkar_trollAI(Creature* pCreature) : ScriptedAI(pCreature) {}
    ...
    void SpellHit(Unit* pCaster, const SpellEntry* pSpell)
    {
        if (pCaster->GetTypeId() == TYPEID_PLAYER && pSpell->Id == SPELL_AHUNAES_KNIFE && pCaster->ToPlayer()->GetQuestStatus(QUEST_SCALPS) == QUEST_STATUS_INCOMPLETE && !scalped && me->isDead())
        {
            pCaster->ToPlayer()->KilledMonsterCredit(CREDIT_SCALP,0);
            scalped = true;
        }
    }
};

CreatureAI* GetAI_npc_heb_drakkar_troll(Creature* pCreature)
{
    return new npc_heb_drakkar_trollAI (pCreature);
}
...
void AddSC_zuldrak()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_heb_drakkar_troll";
    newscript->GetAI = &GetAI_npc_heb_drakkar_troll;
    newscript->RegisterSelf();
