...
/*######
## npc_niffelem_frost_giant
######*/

enum eNiffelemFrostGiant
{
    QUEST_GOING_BEARBACK    = 12851,
    SPELL_FLAMING_ARROW_AURA     = 54798,
    CREDIT_FROST_GIANT = 29597,
    CREDIT_FROST_WORG = 29595,
    ENTRY_FROST_GIANT = 29351,
    ENTRY_FROST_WORG = 29358
};

struct npc_niffelem_frost_giantAI : public ScriptedAI
{
    npc_niffelem_frost_giantAI(Creature* pCreature) : ScriptedAI(pCreature) {}
    ...
    void UpdateAI(const uint32 diff)
    {       
        if(m_creature->HasAura(SPELL_FLAMING_ARROW_AURA) && !bBurned)
        {
            Unit * pCaster = m_creature->GetAura(SPELL_FLAMING_ARROW_AURA)->GetCaster()->GetCharmerOrOwnerPlayerOrPlayerItself();
            if(pCaster && pCaster->GetTypeId() == TYPEID_PLAYER && pCaster->ToPlayer()->GetQuestStatus(QUEST_GOING_BEARBACK) == QUEST_STATUS_INCOMPLETE)
            {
                switch(m_creature->GetEntry())
                {
                    case ENTRY_FROST_GIANT:
                        pCaster->ToPlayer()->KilledMonsterCredit(CREDIT_FROST_GIANT, 0);
                    break;
                    case ENTRY_FROST_WORG:
                        pCaster->ToPlayer()->KilledMonsterCredit(CREDIT_FROST_WORG, 0);
                    break;
                }
                bBurned = true;
            }
            m_creature->RemoveAura(SPELL_FLAMING_ARROW_AURA);
        }
        if(bBurned)
        {
            if (burnTimer <= diff)
...
CreatureAI* GetAI_npc_niffelem_frost_giant(Creature* pCreature)
{
    return new npc_niffelem_frost_giantAI(pCreature);
}

/* NOT YET IMPLEMENTET
enum eIcefang
{
    SPELL_ICEFANG_BEGIN_PATHING = 54788,
    SPELL_ICEFANG_PREP_AURA = 61169
};
...
CreatureAI* GetAI_npc_icefang(Creature* pCreature)
{
...
void AddSC_storm_peaks()
{
    ...
    newscript = new Script;
    newscript->Name = "npc_niffelem_frost_giant";
    newscript->GetAI = &GetAI_npc_niffelem_frost_giant;
    newscript->RegisterSelf();

    /*newscript = new Script;
    newscript->Name = "npc_icefang";
    newscript->GetAI = &GetAI_npc_icefang;
    newscript->RegisterSelf();
    */
}
