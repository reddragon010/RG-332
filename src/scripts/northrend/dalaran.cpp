...
#define GOSSIP_ITEM_AOF1 "I have a book that might interest you Would you like to take a look?"
#define GOSSIP_ITEM_AOF2 "Thank you! I will be sure to notify you if I find anythink else"

enum eVargoth
{
	SPELL_CREATE_FAMILIAR		= 61457,
	SPELL_KIRIN_TOR_FAMILIAR	= 61472,

	ITEM_ARCANE_MAGIC_MASTERY	= 43824,
	ITEM_KIRIN_TOR_FAMILIAR		= 44738,

	GOSSIP_TEXTID_VARGOTH1		= 31000,
	GOSSIP_TEXTID_VARGOTH2		= 13304
};

bool GossipHello_npc_archmage_vargoth(Player* pPlayer, Creature* pCreature)
{	
	if (pPlayer->HasItemCount(ITEM_ARCANE_MAGIC_MASTERY, 1) && pPlayer->HasItemCount(ITEM_KIRIN_TOR_FAMILIAR, 1))
	{	
		pPlayer->SEND_GOSSIP_MENU(pPlayer->GetGossipTextId(pCreature), pCreature->GetGUID());
		return true;
	}

	if (pPlayer->HasItemCount(ITEM_ARCANE_MAGIC_MASTERY, 1) && pPlayer->HasSpell(SPELL_KIRIN_TOR_FAMILIAR))
	{
		pPlayer->SEND_GOSSIP_MENU(pPlayer->GetGossipTextId(pCreature), pCreature->GetGUID());
		return true;
	}

	if (pPlayer->HasItemCount(ITEM_ARCANE_MAGIC_MASTERY, 1))
	{
	pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_AOF1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
	    pPlayer->PlayerTalkClass->SendGossipMenu(GOSSIP_TEXTID_VARGOTH1, pCreature->GetGUID());
		return true;
	}
...
bool GossipSelect_npc_archmage_vargoth(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    switch (uiAction)
    {
    case GOSSIP_ACTION_INFO_DEF+1:
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_AOF2, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
        pPlayer->PlayerTalkClass->SendGossipMenu(GOSSIP_TEXTID_VARGOTH2, pCreature->GetGUID());
        break;
    case GOSSIP_ACTION_INFO_DEF+2:
        pPlayer->CastSpell(pPlayer, SPELL_CREATE_FAMILIAR, true);
...
void AddSC_dalaran()
{
	...
	newscript = new Script;
    newscript->Name = "npc_archmage_vargoth";
    newscript->pGossipHello = &GossipHello_npc_archmage_vargoth;
    newscript->pGossipSelect = &GossipSelect_npc_archmage_vargoth;
    newscript->RegisterSelf();
}
