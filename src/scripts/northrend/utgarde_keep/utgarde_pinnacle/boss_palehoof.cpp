...
struct boss_palehoofAI : public ScriptedAI
{
    ...
    void Reset()
    {
        uiArcingSmashTimer = 15*IN_MILISECONDS;
        uiImpaleTimer = 12*IN_MILISECONDS;
        uiWhiteringRoarTimer = 10*IN_MILISECONDS;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (uiArcingSmashTimer <= diff)
        {
            ...
            uiArcingSmashTimer = urand(13*IN_MILISECONDS,17*IN_MILISECONDS);
        ...
        if (uiImpaleTimer <= diff)
        {
          ...
          uiImpaleTimer = urand(8*IN_MILISECONDS,12*IN_MILISECONDS);
        } else uiImpaleTimer -= diff;
        
        if (uiWhiteringRoarTimer <= diff)
        {
            DoCast(me, DUNGEON_MODE(SPELL_WITHERING_ROAR, H_SPELL_WITHERING_ROAR));
            uiWhiteringRoarTimer = urand(8*IN_MILISECONDS,12*IN_MILISECONDS);
...
struct mob_ravenous_furbolgAI : public ScriptedAI
{
    ...
    void Reset()
    {
        uiChainLightingTimer = 5*IN_MILISECONDS;
        uiCrazedTimer = 10*IN_MILISECONDS;
        uiTerrifyingRoarTimer = 15*IN_MILISECONDS;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            DoCast(me->getVictim(), DUNGEON_MODE(SPELL_CHAIN_LIGHTING, H_SPELL_CHAIN_LIGHTING));
            uiChainLightingTimer = 5*IN_MILISECONDS + rand()%5*IN_MILISECONDS;
        } else uiChainLightingTimer -=  diff;

        if (uiCrazedTimer <= diff)
        {
            DoCast(me, SPELL_CRAZED);
            uiCrazedTimer = 8*IN_MILISECONDS + rand()%4*IN_MILISECONDS;
        } else uiCrazedTimer -=  diff;

        if (uiTerrifyingRoarTimer <= diff)
        {
            DoCast(me, SPELL_TERRIFYING_ROAR);
            uiTerrifyingRoarTimer = 10*IN_MILISECONDS + rand()%10*IN_MILISECONDS;
...
struct mob_frenzied_worgenAI : public ScriptedAI
{
    ...
    void Reset()
    {
        uiMortalWoundTimer = 5*IN_MILISECONDS;
        uiEnrage1Timer = 15*IN_MILISECONDS;
        uiEnrage2Timer = 10*IN_MILISECONDS;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (uiMortalWoundTimer <= diff)
        {
            ...
            uiMortalWoundTimer = 3*IN_MILISECONDS + rand()%4*IN_MILISECONDS;
...
struct mob_ferocious_rhinoAI : public ScriptedAI
{
    ...
    void Reset()
    {
        uiStompTimer = 10*IN_MILISECONDS;
        uiGoreTimer = 15*IN_MILISECONDS;
        uiGrievousWoundTimer = 20*IN_MILISECONDS;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (uiStompTimer <= diff)
        {
            ...
            uiStompTimer = 8*IN_MILISECONDS + rand()%4*IN_MILISECONDS;
        } else uiStompTimer -= diff;

        if (uiGoreTimer <= diff)
        {
            DoCast(me->getVictim(), DUNGEON_MODE(SPELL_GORE, H_SPELL_GORE));
            uiGoreTimer = 13*IN_MILISECONDS + rand()%4*IN_MILISECONDS;
        ...
        if (uiGrievousWoundTimer <= diff)
        {
            ...
            uiGrievousWoundTimer = 18*IN_MILISECONDS + rand()%4*IN_MILISECONDS;
...
struct mob_massive_jormungarAI : public ScriptedAI
{
    ...
    void Reset()
    {
        uiAcidSpitTimer = 3*IN_MILISECONDS;
        uiAcidSplatterTimer = 12*IN_MILISECONDS;
        uiPoisonBreathTimer = 10*IN_MILISECONDS;
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (uiAcidSpitTimer <= diff)
        {
            ...
            uiAcidSpitTimer = 2*IN_MILISECONDS + rand()%2*IN_MILISECONDS;
        } else uiAcidSpitTimer -= diff;

        if (uiAcidSplatterTimer <= diff)
        {
            DoCast(me, DUNGEON_MODE(SPELL_POISON_BREATH, H_SPELL_POISON_BREATH));
            uiAcidSplatterTimer = 10*IN_MILISECONDS + rand()%4*IN_MILISECONDS;
        ...
        if (uiPoisonBreathTimer <= diff)
        {
            ...
            uiPoisonBreathTimer = 8*IN_MILISECONDS + rand()%4*IN_MILISECONDS;
