...
enum eSturdyvine
{
    ...
    GO_PAPAYA                   = 190623,
    GO_ORANGE                   = 190624,    
    GO_BANANA_BUNCH             = 190625
};

bool GOHello_go_sturdy_vine(Player *pPlayer, GameObject *pGO)
{
    if (pPlayer->GetQuestStatus(QUEST_SOME_MAKE_LEMONADE) == QUEST_STATUS_INCOMPLETE)
    {
        pGO->UseDoorOrButton();
        if(GameObject * temp = pPlayer->SummonGameObject(urand(GO_PAPAYA, GO_BANANA_BUNCH), pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 0, 0, 0,0 ,0, 0))
        {
            temp->SetRespawnTime(60);
        }
    }
    return true;
} 
