...
npc_experiencie_eliminator 80%  Disable XP Gain - Missing Emotes
EndContentData */
...
struct npc_winter_revelerAI : public ScriptedAI
{
    ...
    void ReceiveEmote(Player* pPlayer, uint32 emote)
    {
        ...
        if (emote == TEXTEMOTE_KISS && !pPlayer->IsMounted())
        {
...
#define GOSSIP_EE_DISABLE           100006
#define GOSSIP_EE_DISABLE_CONFIRM   100007
#define GOSSIP_EE_ENABLE            100008
#define GOSSIP_EE_ENABLE_CONFIRM    100009
#define GOSSIP_EE_EMOTE_1           -1999991
#define GOSSIP_EE_EMOTE_2           -1999992
#define GOSSIP_EE_EMOTE_3           -1999993
#define GOSSIP_EE_EMOTE_4           -1999994

struct npc_experiencie_eliminatorAI : public ScriptedAI
{
    npc_experiencie_eliminatorAI(Creature *c) : ScriptedAI(c) {Reset();}
    uint32 emoteTimer;

    void Reset() {
        emoteTimer = urand(360000, 480000); //6 to 8min
    }
    ...
    void UpdateAI(const uint32 diff) 
    {
        if (emoteTimer <= diff)
        {
            ...
            {
                case 0:
                    DoScriptText(GOSSIP_EE_EMOTE_1, m_creature);
                    break;
                case 1:
                    DoScriptText(GOSSIP_EE_EMOTE_2, m_creature);
                    break;
                case 2:
                    DoScriptText(GOSSIP_EE_EMOTE_3, m_creature);
                    break;
                default:
                    DoScriptText(GOSSIP_EE_EMOTE_4, m_creature);
                    break;
            }
            emoteTimer = urand(360000, 480000);
        } emoteTimer -= diff;
...
CreatureAI* GetAI_npc_experiencie_eliminator(Creature* pCreature)
{
    return new npc_experiencie_eliminatorAI (pCreature);
}
...
bool GossipHello_npc_experience_eliminator(Player *pPlayer, Creature *pCreature)
{
    if(!pPlayer->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_NO_XP_GAIN))
        pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT,GOSSIP_EE_DISABLE,GOSSIP_SENDER_MAIN,GOSSIP_ACTION_INFO_DEF + 1,GOSSIP_EE_DISABLE_CONFIRM,100000,false);
	else
		pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT,GOSSIP_EE_ENABLE,GOSSIP_SENDER_MAIN,GOSSIP_ACTION_INFO_DEF + 2,GOSSIP_EE_ENABLE_CONFIRM,100000,false);
...
void AddSC_npcs_special()
{
    ...
    newscript->GetAI = &GetAI_npc_experiencie_eliminator;
