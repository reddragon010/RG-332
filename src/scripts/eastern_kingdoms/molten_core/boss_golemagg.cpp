...
struct boss_golemaggAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if (m_pInstance)
            m_pInstance->SetData(DATA_GOLEMAGG_DEATH, NOT_STARTED);
    }
    ...
    void EnterCombat()
    {
        DoZoneInCombat();
        if (m_pInstance)
            m_pInstance->SetData(DATA_GOLEMAGG_DEATH, IN_PROGRESS);
    }

    void JustDied(Unit* pKiller)
    {
        if (m_pInstance)
            m_pInstance->SetData(DATA_GOLEMAGG_DEATH, DONE);
    }
            ...
            if (m_pInstance)
            {
                if (Creature* pGolemagg = m_pInstance->instance->GetCreature(m_pInstance->GetData64(DATA_GOLEMAGG)))
                {
                    if (pGolemagg->isAlive())
                    {
                        ...
                        Position pos;
                        me->GetPosition(&pos);
                        if (!pGolemagg->IsInDist(&pos, 20))
                            pGolemagg->CastSpell(me, SPELL_GOLEMAGG_TRUST, false);
                    }
                    else
