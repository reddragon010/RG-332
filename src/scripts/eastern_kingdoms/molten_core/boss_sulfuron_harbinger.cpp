...
struct boss_sulfuronAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if (pInstance)
            pInstance->SetData(DATA_SULFURON_EVENT, NOT_STARTED);
    }
    ...
    void EnterCombat(Unit *who)
    {
        DoZoneInCombat();
        if (pInstance)
            pInstance->SetData(DATA_SULFURON_EVENT, IN_PROGRESS);
    }

    void JustDied(Unit* killer)
    {
        if (pInstance)
            pInstance->SetData(DATA_SULFURON_EVENT, DONE);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            DoCast(SPELL_DEMORALIZINGSHOUT);
