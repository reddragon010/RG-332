...
#define SPELL_GARR_ARMOR_DEBUFF     20481
#define SPELL_SUMMON_PLAYER         20477
...
struct boss_garrAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if (pInstance)
            pInstance->SetData(DATA_GARRISDEAD, NOT_STARTED);
    }
    ...
    void EnterCombat(Unit *who)
    {
        DoZoneInCombat();
        if (pInstance)
            pInstance->SetData(DATA_GARRISDEAD, IN_PROGRESS);
    }

    void JustDied(Unit* killer)
    {
        if (pInstance)
            pInstance->SetData(DATA_GARRISDEAD, DONE);
    }
