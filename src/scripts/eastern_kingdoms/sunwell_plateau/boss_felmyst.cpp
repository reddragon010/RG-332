...
struct boss_felmystAI : public ScriptedAI
{
    ...
    void Reset()
    {
		...
		const CreatureInfo* cinfo = ((Creature*)m_creature)->GetCreatureInfo();
		if (cinfo)
			m_creature->SetDisplayId(cinfo->Modelid1);
    ...
    void EnterPhase(PhaseFelmyst NextPhase)
    {
        ...
        {
        case PHASE_GROUND:
			m_creature->CastStop(SPELL_FOG_BREATH);
            ...
            case EVENT_SUMMON_FOG:
                {
                    ...
                    if (Creature *Fog = m_creature->SummonCreature(MOB_VAPOR_TRAIL, x, y, z, 0, TEMPSUMMON_TIMED_DESPAWN, 10000))
                    {
						...
						m_creature->CastSpell(Fog, SPELL_FOG_FORCE, true);
                    }
                }
